...
/* Ende JQuery_UI_a11y */
/* Start Addon_FlyoutMenu */
(function($){
jQuery.init_flyout = function(id){
    var menu = $(id);
    menu.find('li').not('li li').attr('tabindex','-1').each(function(){
        $(this).hover(function(){
          $(this).addClass('hovered').find('.menu-box').addClass('visible');
          ...
          $(this).find('.col').height(tempHeight);
          
          var tempWidth = $(this).width();
          var currentElement = $(this).find('div.menu-box');
       if(($(currentElement).width()- tempWidth) < '20'){
              $(currentElement).width(tempWidth-3);
          }
          
          
        },
        function(){
            $(this).removeClass('hovered').find('.menu-box').removeClass('visible');
            $(this).closest('ul').find('li').removeClass('hovered').end();
            $(this).closest('ul').find('.menu-box').removeClass('visible');
        });
    });
    menu.find('li a').not('li li a').each(function(){
        $(this).focus(function(){
            menu.find('li').removeClass('hovered');
            
            var tempHeight = 0;
            
            $(this).parent().find('.col').each(function() {
              if($(this).height() > tempHeight)
                tempHeight = $(this).height();
            });
            $(this).find('.col').height(tempHeight);
            ...
            $(this).closest('ul').find('.menu-box').removeClass('visible');
            $(this).closest('li').addClass('hovered').find('.menu-box').addClass('visible');
        });
    });
    menu.find('li:last a').each(function(){
        $(this).blur(function(){
            menu.find('li').removeClass('hovered').find('.menu-box').removeClass('visible');
           
        });
    });
};
})(jQuery);
/* Ende Addon_FlyoutMenu */
/* Start Addon_Calendar */
/**
...
(function($) {
 ...
 $.location = (function() {
  ...
  function strToObj(str, opts) {
   ...
   if (str) {
    if ('&'.replace(opts.seperator, '1') === '1') {
     paramBegin = str.indexOf('?');
     str = (paramBegin > -1) ? str.substr(paramBegin + 1) : str;
     str = str.replace(/&amp;/g, '&');
    }
    ...
    $
      .each(
        ...
        function(i, param) {
         ...
         if (opts.decode) {
          obj[decodeURIComponent(queryPair[0])
            .replace(/\+/g, ' ')] = (queryPair[1]) ? decodeURIComponent(
            queryPair[1]).replace(/\+/g,
            ' ')
            : '';
         } else {
  ...
  function objToStr(obj, opts) {
   ...
   if (opts.decode) {
    for (prop in obj) {
     strAr.push((obj[prop]) ? encodeURIComponent(prop).replace(
       ' ', '+')
       + opts.assignator
       + encodeURIComponent(obj[prop]).replace(' ', '+')
       : encodeURIComponent(prop).replace(' ', '+'));
    }
   } else {
    for (prop in obj) {
     strAr.push((obj[prop]) ? prop + opts.assignator + obj[prop]
       : prop);
    }
   }
...
(function($) {
 ...
 $.fn.dialogShow = function(time) {
  return this
    .each(function() {
     var jElm = $(this), parent = jElm.parent(), startLeft = parent[0].offsetLeft
       - (parent.width() / 2);
     jElm.css({
      left : startLeft,
      height : jElm.height()
 ...
 $.fn.dialogHide = function(time) {
  return this
    .each(function() {
     var jElm = $(this), parent = jElm.parent(), endLeft = parent[0].offsetLeft
       - (parent.width() / 2);
     jElm.css({
      left : 0,
      height : jElm.height()
...
(function($) {
 var calendar, liveCalendar;
 function createCalendarDetails() {
  var dialogs, hideTimeout;
  function init() {
   dialogs = [];
   liveCalendar.find('> table div.calendar-details').each(addBindings);
   dialogs = $(dialogs);
  }
  function addBindings() {
   var selfHideTimeout, dialog = $(this).hide().attr({
    role : 'dialog',
    'aria-hidden' : 'true'
   }), tr = dialog.parent().inOut(showDialog, hideDialog, {
    useEventTypes : 'mouse',
    mouseDelay : 300
    ...
    selfHideTimeout = setTimeout(function() {
     dialog.stop(true, true).dialogHide(200);
    }, 10);
   }).data('dialog', dialog);
   tr.children('a').data('dialog', dialog).attr({
    'aria-haspopup' : 'true',
  ...
  function showDialog() {
   var jElm = $(this), dialog = jElm.data('dialog');
   if (dialog && dialog.is(':hidden')) {
    dialogs.stop(true, true).dialogHide(200);
    dialog.stop(true, true).dialogShow(200).find('h4').attr({
     'tabIndex' : '-1'
    }).setFocus(180);
    return false;
   }
  ...
  liveCalendar.bind('calendarUpdate', init).inOut(removeHide, hideAll, {
   mouseDelay : 200,
   keyDelay : 100,
   bothOut : true
  });
 ...
 function switchCalendarMonth() {
  var stateInfo, stateText = 'Lade Kalender';
  liveCalendar.bind('calendarUpdate', init);
  function init() {
   var goToButtons = calendar.find('div.pager a').attr({
    role : 'button'
   }).bind('click', loadCalendar);
  }
  function showNewCalendar(data) {
   liveCalendar.html(data).trigger('calendarUpdate').dequeue()
     .animate({
      opacity : 1
     }, 300).find('h2:first').attr({
      'tabIndex' : '-1'
     }).setFocus({
      time : 180,
      complete : function() {
  ...
  function loadCalendar() {
   stateInfo.hide().ariaShow().text(stateText);
   liveCalendar.animate({
    opacity : 0
   }, 300);
   var url = $(this).attr('href'), paramBegin = url.indexOf('?'), baseUrl = (paramBegin > -1) ? url
     .substr(0, paramBegin + 1)
     : url, params = (paramBegin > -1) ? $.location
     .strToObj(url) : {};
   ;
   params.view = 'renderCalendar';
   if ('cms_param3' in params) {
    delete params.cms_param3;
   }
   baseUrl = calendar_link;
   $.ajax({
    url : baseUrl,
    data : params,
    success : function() {
     liveCalendar.queue($.proxyFn(showNewCalendar, this,
       arguments));
    }
   });
   return false;
  }
  stateInfo = $('<span class="calendar-state" />').hide().attr({
   'role' : 'alert',
   'aria-hidden' : 'true',
   'aria-live' : 'polite'
...
var bodyClass;
function jumpUp() {
  if (activeScroller) window.clearInterval(activeScroller);
  window.scrollTo(0,0);
  startY=0;
  thisY=0;
  speedY=0;
  bodyTag.style.width="680px";
}
function scrollUp() {
  thisY=thisY-speedY;
  thisX=thisX-speedX;
  ...
  var sWidth=""+thisX+"px";
  bodyTag.style.width=""+thisX+"px";
}
function setPrintCssMedia(newValue)
{
  var ppcss=document.getElementById("printcss");
  if (ppcss)
  {
       ppcss.setAttribute("media",newValue)
  }
}
function ppToggle() {
       if (ppStatus==1){
         //Umschalten auf Druckvorschau
         ppOn();      
       } else {
          //Umschalten auf Normalansicht
         ppOff()
       }     
}
function ppOff() {
  ppStatus=1;
  setPrintCssMedia("print");
  bodyTag.style.width="auto";
  bodyTag.className=bodyClass;
}
function ppOn() {
   bodyTag=document.getElementsByTagName("body")[0];
   document.getElementById("printPreview").scrollIntoView();
   bodyClass=bodyTag.className;
   bodyTag.className="printpreview";
   ppStatus=2;
  setPrintCssMedia("print, screen");
  thisY=window.pageYOffset;
  speedY=thisY/30;
  thisX=window.innerWidth;
  xDiff=thisX-630;
  speedX=xDiff/30;
  bodyTag.style.width="auto";
  //activeScroller=window.setInterval("scrollUp()", 40);
  //  window.setTimeout("jumpUp()",3000);
  jumpUp();
  //setTimeout(function(){    window.resizeBy(10,10);    }, 1000 ); 
}
...
jQuery('ServicebereichSuche').attachToField('topSearchField','');
return;
};
...
jQuery.init_glossarylink = function() {
    ...
    $('.GlossarEntry').removeAttr('onclick').click(function(){
...
/* Start Addon_Handheld */
function setMediaResize () {
 $('link[rel*=style]').each(function(i)  {
        var med = $(this).attr('media');
        if (med && med.indexOf('only screen and (max-device-width: 640px)')> -1) {
         var currentItem = $(this);
      
         if ($(window).width() <= 640) {      
      $(currentItem).removeAttr('disabled').attr('media','screen, handheld, projection');
        ...
        if (med && med.indexOf('screen, handheld, projection')> -1) {
         var currentItem = $(this);
      
         if ($(window).width() <= 640) {
   
         } else {
          $(currentItem).removeAttr('disabled').attr('media','only screen and (max-device-width: 640px)');
          $(this).attr('disabled','disabled');
         };
        };
...
/*
(function($){
 jQuery.init_handheld = function(id){
     return $('#navServiceMeta, #navService').find('li').each(function() {
        $(this).clone().appendTo('#navFunctions');
     }).end().remove();
  };
 })(jQuery);
 */
...
var request = null;
 ...
 this.suchanweisung = function()
  ...
  /*
   * Vorbelegung fuer das Suchfeld
   */
  var defaultText = "Suche in GENESIS...";
 ...
 function holeStichwortePerAJAX()
  ...
  if( ( ( field.value.length > 2 ) && ( field.value.lastIndexOf( ' ' )<0 ) )
   ...
   if( request )
    ...
    field.focus();
    var url = "https://www-genesis.destatis.de/genesis/ajax";
...
/* Ende GenesisAjax */
/* Start jquery.scrollButtons */
/*!
 * scrollButtons - v0.1 - 14/04/2011
 *
 ...
 * Usage:
 ***********************
 * $(".interaktiveAnwendungen .alleAnwendungen").scrollButtons();
 * $(".interaktiveAnwendungen .alleAnwendungen").scrollButtons({
 *   elementsVisible : 3
 * });
 *
...
(function($) {
  $.fn.extend({
    scrollButtons: function(options) {
      var defaults = {
        scrollSpeed     : 400,
        naviHTML        : '<div class="miniNav"><ul><li class="btnLeft"><a title="'+messageZurueck+'" href="javascript:void(0)"><span class="aural">'+messageZurueck+'</span></a></li><li class="btnRight"><a title="'+messageVor+'" href="javascript:void(0)"><span class="aural">'+messageVor+'</span></a></li></ul></div>',
        elementClass    : 'teaser',
        elementsVisible : 3
      };
      ...
      return this.each(function() {
        ...
        var naviHTML = options.naviHTML;
        var elementClass = options.elementClass;
        var elementsVisible = options.elementsVisible;
    
        var curScrollPos = 0;
        var maxScrollLeft = 0;
        var elementCount = obj.find("." + elementClass).length;
        var elementWidth = 0;
    
        // dont continue if no overflow
        if(elementCount <= elementsVisible) return;
    
        // insert navi
        obj.before(naviHTML);
        var navi = obj.prev(".miniNav");
        var btnRight = navi.find(".btnRight");
        var btnLeft = navi.find(".btnLeft");
        ...
        function init() {
          ...
          obj.scrollLeft(99999);
          maxScrollLeft = obj.scrollLeft();
        
          elementWidth = Math.round(maxScrollLeft / (elementCount - elementsVisible));
        
          maxScrollLeft = elementWidth * (elementCount - elementsVisible);
          ...
          btnRight.click(function() {
            if((curScrollPos + elementWidth) <= maxScrollLeft) {
              curScrollPos += elementWidth;
              obj.stop().animate({
                scrollLeft: curScrollPos
              }, scrollSpeed);
            ...
            if(curScrollPos >= maxScrollLeft)
              $(this).addClass("disabled");
          
          });
      
          btnLeft.click(function() {
            if((curScrollPos - elementWidth) >= 0) {
              curScrollPos -= elementWidth;
              obj.stop().animate({
                scrollLeft: curScrollPos
              }, scrollSpeed);
...
/* Ende jquery.scrollButtons */
/* Start jquery.scrollTabs */
/*!
 * scrollTabs - v0.2 - 06/01/2012
 *
 * Copyright (c) 2011 ]init[ AG
 * Author: Sebastian Baranowski
 */
  ...
  $.fn.extend({
    scrollTabs: function(options) {
      ...
      var defaults = {
        scrollSpeed     : 400,
        scrollWidth     : 180,
        naviHTML        : '<div class="btnLeft"><a href="javascript:void(0);"><span class="aural">zurück</span></a></div><div class="btnRight"><a href="javascript:void(0);"><span class="aural">vor</span></a></div>',
        elementClass    : 'level1'
      };
      ...
      return this.each(function() {
        ...
        function init() {
          ...
          if(btnRight && btnLeft) {
            btnRight.parent().remove();
            btnLeft.parent().remove();
          }
          obj.css("margin-left", "26px").css("margin-right", "26px");
          var curScrollPos = 0;
          obj.scrollLeft(99999);
          var maxScrollLeft = obj.scrollLeft();
          var activeTabPos = 0;
          // dont continue if no overflow
          if(maxScrollLeft === 0) {
            obj.css("margin-left", "0px").css("margin-right", "0px");
            return;
          }
          // insert navi
          obj.before(naviHTML);
          btnRight = obj.parent().find(".btnRight a");
          btnLeft = obj.parent().find(".btnLeft a");
          itemClick = obj.find("li a");
          // hide scrollbar
          obj.css("overflow", "hidden");
          btnRight.unbind('click');
          btnLeft.unbind('click');
          ...
          btnRight.removeClass("disabled");
          // Functions
          function setScrollLeft(positionLeft){
            obj.stop().animate({
              scrollLeft: positionLeft
            }, scrollSpeed);
          }
          function initializeTabItems() {
            var tabItems = new Array(); 
            obj.find("li").each(
              function(){
                tabItems.push([$(this).index(), $(this).width()]);
              }
            );
            return tabItems;
          }
          ...
          function resetSlider(){
            curScrollPosItem = (obj.find("li.current").index());
            var tabItems = initializeTabItems();
            var positionLeft = 0;
            for (var i=0; i<curScrollPosItem; i++){   
              positionLeft += (tabItems[i][1]+11);
              if (positionLeft >= maxScrollLeft){
                curScrollPos = i;
                break;
              }
            }
            if (positionLeft <= maxScrollLeft){
              setScrollLeft(positionLeft);
              curScrollPos = curScrollPosItem;
            } else{
              setScrollLeft(positionLeft);
          ...
          itemClick.click(function(){
            var tabItems = initializeTabItems();
            var positionLeft = 0;
            var curScrollPosItem = ($(this).parents("li").index());
            for (var i=0; i<curScrollPosItem; i++){   
            positionLeft += (tabItems[i][1]+11);
            if (positionLeft >= maxScrollLeft){
             curScrollPos = i;
...
/* Ende jquery.scrollTabs */
/* Start jquery.fancybox-1.3.4.pack */
/*
...
/* Ende jquery.fancybox-1.3.4.pack */
var calendar_link = "SiteGlobals/Functions/Kalender/DE/Kalender_Marginal.html?nn=142234";
jQuery(document).ready(function (){
  $ ('body').addClass('js-on');
  $ .init_clearfields();
  $ .init_printlink('#navFunctions');
  $ .init_flyout('#navPrimary');
  $ .init_glossarylink();
  $ .init_calendar();
